...
const int Params = 10;
const int mBoneMatrixArray = 11;

const int VSBUFFER_SIZE = 11 + MAX_MATRICES * 3;
...
attribute mediump vec4	BlendWeight;
attribute mediump vec4 BlendIndices;
...
{
	mediump float fogStart = fogParams[0];
	mediump float fogEnd = fogParams[1];
	mediump float fogDensity = fogParams[2];
	
	mediump float fogCoeff = clamp((d - fogStart) / (fogEnd - fogStart), float(0.0), float(1.0));
...
void main(void)
{
	...
	{
		v_Tex0.z = CalcFogFactor(cameraPos.z, g_vsbuffer[FogParams]);
	}
	else
	{
		v_Tex0.z = 0.0;
