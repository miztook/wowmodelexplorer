...
namespace WowMapExplorer.Commands
{
    public class OptionCommands
    {
        ...
        private void ShowOptionExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            bool show = MapEditorService.Instance.IsShow(option.Value);
            switch (option.Value)
            {
                ...
                case EditorShowOptions.Coord:
                    MapSceneService.Instance.CoordSceneNode.Visible = !show;
                    break;
            }
            MapEditorService.Instance.SetShow(option.Value, !show);
        }
