...
void processPan2( const SGestureInfo& gesInfo )
{
	...
	{
		float x = gesInfo.param.pan2_xPos;
		float y = gesInfo.param.pan2_yPos;
...
{	
	...
	else if (gesInfo.state == GestureState_Change)
	{
		...
		cam->onKeyMove(max_(1.0f, abs_(gesInfo.param.pinch_scale - lastScale)) * 2.0f, keyControl);
#else
		cam->onKeyMove(max_(1.0f, abs_(gesInfo.param.pinch_scale - lastScale)) * 20.0f, keyControl);
#endif
	}
	else
	{
