...
wow_tileScene::wow_tileScene( CMapTileSceneNode* mapTileNode ) : TileSceneNode(mapTileNode)
{
	...
	uint32_t numM2Instance = TileSceneNode->Block.tile->fileAdt->NumM2Instance;
	uint32_t numWmoInstance = TileSceneNode->Block.tile->fileAdt->NumWmoInstance;
...
void wow_tileScene::addM2SceneNodes()
{
	...
	for (uint32_t i=0; i<adt->NumM2Instance; ++i)
	{
		const IFileADT::SM2Instance* instInfo = adt->getM2Instance(i);
		uint32_t index = instInfo->m2Index;

		const char* filename = adt->getM2FileName(index);
...
void wow_tileScene::addWMOSceneNodes()
{
	...
	for (uint32_t i=0; i<adt->NumWmoInstance; ++i)
	{
		const IFileADT::SWmoInstance* instInfo = adt->getWMOInstance(i);
		uint32_t index = instInfo->wmoIndex;

		const char* filename = adt->getWMOFileName(index);
...
void wow_tileScene::registerVisibleM2Instances( ICamera* cam )
{
	...
	float objectVisibleDistance = g_Engine->getSceneRenderServices()->getObjectVisibleDistance();
	float objectAlphaDistance = g_Engine->getSceneRenderServices()->getObjectAlphaDistance();
	...
	for (uint8_t i=0; i<16; ++i)
	{
		for (uint8_t j=0; j<16; ++j)
		{
			...
			for (T_InstanceList::const_iterator itr=littleM2Instances.begin(); itr != littleM2Instances.end(); ++itr)
			{
				...
				float radius = M2InstSceneNodes[idx].radius;
				if (radius == 0.0f)
					continue;

				float distance = cam->getPosition().getDistanceFrom(center) / radius;
				...
				float delta = distance - objectAlphaDistance;
				if (delta > 0)
...
void wow_tileScene::classifyM2Instance( uint32_t index, const matrix4& instMat )
{
	CFileADT* adt = static_cast<CFileADT*>(TileSceneNode->Block.tile->fileAdt);
...
void wow_tileScene::processResources()
{
	...
	if(loader->m2LoadCompleted())
	{
		...
		if(!filem2 || TileSceneNode != (IMapTileSceneNode*)task.param.param1)		//不是需要的m2
			return;
		uint32_t index = PTR_TO_UINT32(task.param.param2);
