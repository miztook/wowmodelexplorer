...
int main()
{
	...
	//_CrtSetBreakAlloc(126992);

	CMemDbg globalDbg;
	globalDbg.beginCheckPoint();

	SWindowInfo wndInfo = Engine::createWindow("App8", dimension2du(800,600));
	HWND hwnd = wndInfo.hwnd;

	SEngineInitParam param;
	createEngine(param, wndInfo);

	bool success = g_Engine->initDriver(EDT_DIRECT3D9, 0, false, true, 1, true);
	g_bExit = !success;
