...
{	
	...
	else if (gesInfo.state == GestureState_Change)
	{
		...
		cam->onKeyMove(max_(1.0f, abs_(gesInfo.param.pinch_scale - lastScale)) * 0.2f, keyControl);
#else
		cam->onKeyMove(max_(1.0f, abs_(gesInfo.param.pinch_scale - lastScale)) * 1.0f, keyControl);
#endif
	}
	else
	{
