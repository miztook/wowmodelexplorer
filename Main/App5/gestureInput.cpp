...
void processTap1(const SGestureInfo& gesInfo)
{
	...
	{
		s16 idx = g_DeathWingSceneNode->getFileM2()->getAnimationIndex("Stand", 3);
		if (idx != -1 && g_DeathWingSceneNode->getCurrentAnimationIndex() != idx)
			g_DeathWingSceneNode->playAnimationByIndex(idx, false);
	}
}
...
{	
	...
	else if (gesInfo.state == GestureState_Change)
	{
		...
		cam->onKeyMove(fabs(gesInfo.param.pinch_scale - lastScale) * 10, keyControl);
	}
	else
	{
		lastScale = 1.0f;
