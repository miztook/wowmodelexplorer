...
public:
	...
	quaternion(const vector3df& vec) { fromEuler( vec ); }
	...
	inline quaternion operator*(f32 s) const { return quaternion(s*X, s*Y, s*Z, s*W); }
	inline quaternion& operator*=(f32 s) { X*=s; Y*=s; Z*=s; W*=s; return *this; }
	vector3df operator*(const vector3df& v) const;
	inline quaternion& operator*=(const quaternion& other) { return (*this = other * (*this)); }
	...
	void getRotationMatrix( matrix4 &dest ) const;			//和angle旋转相反
...
inline void quaternion::getRotationMatrix( matrix4 &dest ) const
{
