...
{
	MPQFile* file = g_Engine->getWowEnvironment()->openFile("World\\dnc.db");
	if (!file)
	{
		_ASSERT(false);
		return;
...
void CSceneEnvironment::readOutDoorLightEntry( MPQFile* file, SOutDoorLightEntry* outdoorlight )
{
...
{
	s32 pos = -1;
	u32 numKeys = OutDoorLights.size();
	if (numKeys > 1)
	{
		s32 last = numKeys - 1;
