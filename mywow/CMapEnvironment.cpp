...
{
	lightDB* db = g_Engine->getWowDatabase()->getLightDB();
	//lightIntBandDB* bandDB = g_Engine->getWowDatabase()->getLightIntBandDB();
	...
	for (u32 i=0; i<db->getNumRecords(); ++i)
	{
		...
		if (MapId == r.getInt(lightDB::Map))
		{
			...
			s32 firstId = r.getInt(lightDB::DataIDs) * 18 - 17;
			...
			light->colorRows[SKY_COLOR_0][0].set(0, SColor(20,20,20).color);
			light->colorRows[SKY_COLOR_1][0].set(0, SColor(30,30,30).color);
			light->colorRows[SKY_COLOR_2][0].set(0, SColor(40,40,40).color);
			light->colorRows[SKY_COLOR_3][0].set(0, SColor(60,60,60).color);
			light->colorRows[SKY_COLOR_4][0].set(0, SColor(80,80,80).color);
			light->colorRows[FOG_COLOR][0].set(0, SColor(92,92,92).color);

			if (light->position.isZero() && !GlobalLight)
				GlobalLight = light;
			else
				LocalSkyLights.push_back(SSkyLightEntry(light));
		}
	}
...
bool CMapEnvironment::findSkyWeights(vector3df pos)
{
	...
	for (u32 i=cs+1; i<LocalSkyLights.size(); ++i)
		LocalSkyLights[i].weight = 0.0f;
