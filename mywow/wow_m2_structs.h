...
#pragma  pack(push,1)
	...
	struct header { 				      
		...
		/*0x0A0*/   aabbox3df      _vertexBox;		     //collision 																        						     
		/*0x0B8*/   f32     _vertexRadius;
		/*0x0BC*/   aabbox3df      _boundingbox;                  //bounding
		/*0x0D4*/   f32     _boundingRadius; 
	...
	struct vertex {
		...
		/*0x20*/    vector2df     _TextureCoords;	  	     // Coordinates for a texture.
		/*0x28*/    f32     _Unknown[2];        	     // Null?	
	};
	...
	struct animseq {
		...
		/*0x3E*/    u16  _Index;                        // Id in the list of animations.	
	};
	...
	static const char * _interpnames[] = { "none", "linear", "hermite" };
	//------------------------------------------------------------------------------
	struct animblock {
	...
	enum BlendModes {
		...
		BM_FORCE_32_BIT_DO_NOT_USE = 0x7fffffff
	};
