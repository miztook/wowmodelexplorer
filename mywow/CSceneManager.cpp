...
CSceneManager::CSceneManager( )
	...
	Perf_terrainTime(0), Perf_meshTime(0), Perf_effectTime(0), Perf_wireTime(0),
	Perf_GPUTime(0)
{
	ActiveCamera = NULL;
...
void CSceneManager::drawAll()
{
	...
	Driver->clear(true, true, BackgroundColor);
	...
	beginPerf(calcPerf);
	SceneRenderServices->renderAll(ERT_STATIC_MESH, ActiveCamera);
...
// 			g_Engine->getParticleSystemServices()->getActiveParticlesCount());
		sprintf_s(Text, MAX_TEXT_LENGTH, 
			"\nregister time: %d\ntick time: %d\nrender time: %d\nterrain time: %d\nmesh time: %d\neffect time: %d\nwire time: %d\nGPU time: %d\n",
			Perf_registerTime, Perf_tickTime, Perf_renderTime, Perf_terrainTime, Perf_meshTime, Perf_effectTime, Perf_wireTime, Perf_GPUTime);
		Driver->drawDebugInfo(Text);
	}
