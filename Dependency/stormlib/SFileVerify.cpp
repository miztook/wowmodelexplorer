    ...
    LPBYTE pMd5Digest)
{
    ...
    pbDigestBuffer = STORM_TEMP_ALLOC(BYTE, MPQ_DIGEST_UNIT_SIZE);
    ...
    STORM_TEMP_FREE(pbDigestBuffer);
    return true;
}
    ...
    DWORD dwDataSize)
{
    ...
    pbDataChunk = STORM_TEMP_ALLOC(BYTE, dwChunkSize);
    if(pbDataChunk == NULL)
        return ERROR_NOT_ENOUGH_MEMORY;

    // Allocate space for MD5 array
    pbMD5Array1 = STORM_TEMP_ALLOC(BYTE, dwMD5Size);
    pbMD5Array2 = STORM_TEMP_ALLOC(BYTE, dwMD5Size);
    ...
    if(pbMD5Array2 != NULL)
        STORM_TEMP_FREE(pbMD5Array2);
    if(pbMD5Array1 != NULL)
        STORM_TEMP_FREE(pbMD5Array1);
    if(pbDataChunk != NULL)
        STORM_TEMP_FREE(pbDataChunk);
    return nError;
}
