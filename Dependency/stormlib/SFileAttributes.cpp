...
int SAttrLoadAttributes(TMPQArchive * ha)
{
    ...
    if(SFileOpenFileEx((HANDLE)ha, ATTRIBUTES_NAME, SFILE_OPEN_ANY_LOCALE, &hFile))
    {
        ...
        {
            LPDWORD pArrayCRC32 = STORM_TEMP_ALLOC(DWORD, dwBlockTableSize);
        ...
        {
            ULONGLONG * pArrayFileTime = STORM_TEMP_ALLOC(ULONGLONG, dwBlockTableSize);
            ...
            if(pArrayFileTime != NULL)
            {
                ...
                STORM_TEMP_FREE(pArrayFileTime);
            }
            else
                nError = ERROR_NOT_ENOUGH_MEMORY;
        }
        ...
        {
            unsigned char * pArrayMD5 = STORM_TEMP_ALLOC(unsigned char, (dwBlockTableSize * MD5_DIGEST_SIZE));
            unsigned char * md5;
        ...
        if(nError == ERROR_SUCCESS && (AttrHeader.dwFlags & MPQ_ATTRIBUTE_PATCH_BIT))
        {
            ...
            pbBitArray = STORM_TEMP_ALLOC(BYTE, dwByteSize);
            if(pbBitArray != NULL)
            {
...
int SAttrFileSaveToMpq(TMPQArchive * ha)
{
    ...
    {
        LPDWORD pArrayCRC32 = STORM_TEMP_ALLOC(DWORD, dwFinalBlockTableSize);
    ...
    {
        ULONGLONG * pArrayFileTime = STORM_TEMP_ALLOC(ULONGLONG, ha->dwFileTableSize);
    ...
    {
        char * pArrayMD5 = STORM_TEMP_ALLOC(char, ha->dwFileTableSize * MD5_DIGEST_SIZE);
