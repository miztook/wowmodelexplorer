    ...
    TFileStream * pNewStream)
{
    ...
    {
        DWORD * SectorOffsetsCopy = (DWORD *)STORM_TEMP_ALLOC(BYTE, hf->SectorOffsets[0]);
        ...
        STORM_TEMP_FREE(SectorOffsetsCopy);
    }
    ...
    if(nError == ERROR_SUCCESS && dwBytesToCopy != 0)
    {
        ...
        pbExtraData = STORM_TEMP_ALLOC(BYTE, dwBytesToCopy);
        if(pbExtraData != NULL)
        {
...
bool WINAPI SFileCompactArchive(HANDLE hMpq, const char * szListFile, bool /* bReserved */)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        if((pFileKeys = STORM_TEMP_ALLOC(DWORD, ha->dwFileTableSize)) != NULL)
