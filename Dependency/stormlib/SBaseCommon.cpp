    ...
    DWORD dwKey)
{
    ...
    if(dwCompressedSize < dwRealSize)
    {
        ...
        pbCompressed = STORM_TEMP_ALLOC(BYTE, dwCompressedSize);
...
int WriteSectorChecksums(TMPQFile * hf)
{
    ...
    pbCompressed = STORM_TEMP_ALLOC(BYTE, dwCrcSize);
    if(pbCompressed == NULL)
        return ERROR_NOT_ENOUGH_MEMORY;
    ...
    DWORD dwChunkSize)
{
    ...
    pbFileChunk = STORM_TEMP_ALLOC(BYTE, dwChunkSize);
    if(pbFileChunk == NULL)
    {
        STORM_FREE(md5_array);
        return ERROR_NOT_ENOUGH_MEMORY;
...
void FreeMPQArchive(TMPQArchive *& ha, bool bFreePatch)
{
    if(ha != NULL)
    {
        // First of all, free the patch archive, if any
//         if(ha->haPatch != NULL && bFreePatch)
//             FreeMPQArchive(ha->haPatch, bFreePatch);
