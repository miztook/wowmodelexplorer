...
static int LoadMpqPatch_COPY(TMPQFile * hf, TPatchHeader * pPatchHeader)
{
	...
	hf->pPatchHeader = (TPatchHeader *)STORM_TEMP_ALLOC(BYTE, pPatchHeader->dwSizeOfPatchData);
...
static int LoadMpqPatch_BSD0(TMPQFile * hf, TPatchHeader * pPatchHeader)
{
    ...
    pbCompressed = STORM_TEMP_ALLOC(BYTE, cbCompressed);
...
static void ClearMpqPatch(TMPQFile * hf)
{
	if(hf->pPatchHeader != NULL)
	{
		STORM_TEMP_FREE(hf->pPatchHeader);
    ...
    DWORD dwFlags)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
		...
		if (ha != NULL)
		{
			ha->haPatchList[ha->dwPatchCount] = haPatch;
			haPatch->haBase = ha;
			++ha->dwPatchCount;
			return true;
		}
...
bool WINAPI SFileIsPatchedArchive(HANDLE hMpq)
{
    ...
    return (ha->dwPatchCount > 0);
}

bool   WINAPI SFileAddPatchArchive( HANDLE hMpq, HANDLE hPatchMpq, const char * szPatchPathPrefix, DWORD dwFlags )
{
	TMPQArchive * haPatch = (TMPQArchive *)hPatchMpq;
	TMPQArchive * ha = (TMPQArchive *)hMpq;
	char szPatchPrefixBuff[MPQ_PATCH_PREFIX_LEN];
	int nError = ERROR_SUCCESS;
	...
	// Verify input parameters
	if(!IsValidMpqHandle(ha) || !IsValidMpqHandle(haPatch))
		nError = ERROR_INVALID_HANDLE;
