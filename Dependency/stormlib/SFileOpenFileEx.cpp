...
bool OpenPatchedFile(HANDLE hMpq, const char * szFileName, DWORD dwReserved, HANDLE * phFile)
{
	...
	{
		// Prepare the file name with a correct prefix
        if(SFileOpenFileEx((HANDLE)ha->haPatchList[i], GetPrefixedName(ha->haPatchList[i], szFileName, szPrefixBuffer), SFILE_OPEN_BASE_FILE, &hPatchFile))
        {
        ...
        switch(dwSearchScope)
        {
            ...
            case SFILE_OPEN_BASE_FILE:
                ...
                else
                {
                    ...
                    if(ha->dwPatchCount == 0 || dwSearchScope == SFILE_OPEN_BASE_FILE)
                    {
