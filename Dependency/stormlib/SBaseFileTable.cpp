    ...
    bool bCompress)
{
    ...
    if(bCompress)
    {
        ...
        pCompressed = STORM_TEMP_ALLOC(BYTE, Size);
        if(pCompressed == NULL)
    ...
    LPDWORD pcbTotalSize)
{
    ...
    if(bCompress)
    {
        ...
        // Allocate extra space for compressed table
        pCompressed = (TMPQExtTable *)STORM_TEMP_ALLOC(BYTE, dwTableSize);
        if(pCompressed == NULL)
            return ERROR_NOT_ENOUGH_MEMORY;
    ...
    ULONGLONG FileSize)
{
    ...
    if(nError == ERROR_SUCCESS && pHeader->HiBlockTablePos64 != 0)
    {
        ...
        // Note: pHeader->dwBlockTableSize can be zero !!!
        pHiBlockTable = STORM_TEMP_ALLOC(USHORT, pHeader->dwBlockTableSize + 1);
        if(pHiBlockTable != NULL)
        {
